...
### Added
- **New Icon**: Updated application icon and logo for a more professional look.
- **First Official Release**: TradeSync is now out of beta!

### Fixed
- **Cleanup**: Removed unused assets and aligned repository structure.

## [0.3.2] - 2025-12-14

### Fixed
- **2FA Retry**: Resolved a critical bug that prevented users from retrying login after an incorrect 2FA code entry.
- **UI Improvements**: Various enhancements to the user interface and user experience for smoother interactions.

## [0.3.1] - 2025-12-13

### Fixed
- **Window Dragging**: Resolved an issue where the application window could not be dragged from the top. Implemented a specific drag area at the top of the window for standard behavior.
- **Traffic Lights**: Updated minimize/maximize/close buttons to prevent interference with window dragging.

### Optimized
- **App Size**: Drastically reduced application size from ~200MB to ~48MB by removing heavy dependencies (Pandas/NumPy) and optimizing the build process.
